...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        if use_vq:
            self.vq = VectorQuantize(dim=self.args.n_encoder_hidden, n_embed=codebook_size, decay=vq_decay,
                                     commitment=commitment_weight, eps=1e-5, wait_steps=0, observe_steps=vq_passes)
        else:
            self.vq = nn.Identity()
